...
static void CheckLink(const Database &db, const Link &lk, const std::vector<double> &pkeys, const RawChild &r,
	...
	if (!dup_parent_keys) {
		...
		for (size_t c = 0; c < C; c++) {
			if (parent_of[c] >= 0 && seen[c] == 1) {
				CHECK(csr_parent[c] == parent_of[c], "child %zu under %d, expected %d", c, csr_parent[c],
				      parent_of[c]);
			}
		}
	} else {
	...
	for (int trial = 0; trial < 40; trial++) {
		...
		CHECK(std::fabs(sum - bw.sum) <= 1e-9 * (1 + std::fabs(bw.sum)), "SUM parent %u: got %g want %g", p,
		      sum, bw.sum);
		...
		CHECK(std::fabs(avg - want_avg) <= 1e-9 * (1 + std::fabs(want_avg)), "AVG parent %u: got %g want %g",
		      p, avg, want_avg);
	...
	if (P > 0 && amt_col >= 0) {
		...
		for (int trial = 0; trial < 20; trial++) {
			...
			if (timed) {
				CHECK(never == (all_b.n == 0 ? 1.0f : 0.0f), "never-happened flag parent %u: %g with %zu visible",
				      p, never, all_b.n);
			}
			for (int w = 0; w < kNumWindows; w++) {
				...
				CHECK(n_got == double(bw.n), "window %d count parent %u anchor %.0f: got %g want %zu", w, p,
				      anchor, n_got, bw.n);
				k++;
				...
				if (timed) {
					...
					for (size_t c = 0; c < C; c++) {
						if (parent_of[c] == (int)p && !std::isnan(r.time[c]) && r.time[c] <= anchor &&
						    r.time[c] > lo && r.time[c] > last) {
							last = r.time[c];
						}
					}
					if (bw.n > 0) {
				...
				const double tol = 1e-5 * (1.0 + std::fabs(want_mean)) + 2e-7 * bucket_abs / double(bw.nval ? bw.nval : 1);
				CHECK(std::fabs((double)mean - want_mean) <= tol, "window %d mean parent %u: got %g want %g (n=%zu)",
				      w, p, mean, want_mean, bw.nval);
			}
		...
		for (int trial = 0; trial < 20; trial++) {
			...
			CHECK(std::fabs((double)o[0] - want) <= tol, "SageMeanAt parent %u: got %g want %g (n=%zu)", p, o[0],
			      want, n);
	...
	// put each block back on the link it was fitted on, not on the first match.
	{
		...
		for (size_t i = 0; i < fitted.link_aggs.size() && i < rebound.link_aggs.size(); i++) {
			CHECK(fitted.link_aggs[i].link == rebound.link_aggs[i].link,
			      "two-FK: aggregate %zu fitted on link %d but rebound to link %d", i,
			      fitted.link_aggs[i].link, rebound.link_aggs[i].link);
		}
