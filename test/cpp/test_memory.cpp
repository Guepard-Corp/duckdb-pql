...
static size_t UsableSize(void *p) {
...
static Database MakeDb(uint32_t seed, size_t n_users) {
	...
	for (size_t i = 0; i < n_users; i++) {
		...
		reg.code[i] = (uint32_t)(i % 5) + 1u;
	}
	users.columns = {uid, uts, ten, reg};
	users.nrows = n_users;
	...
	for (size_t i = 0; i < n_users; i++) {
		...
		for (int k = 0; k < n_past; k++) {
			...
			o_ts.push_back(anchor[i] - U(rng) * 300.0 * DAY);
			o_amt.push_back(10.0 + U(rng) * 5.0);
		}
		if (U(rng) < rate[i]) {
			o_uid.push_back(double(i));
			o_ts.push_back(anchor[i] + (1.0 + U(rng) * 25.0) * DAY);
			o_amt.push_back(10.0 + U(rng) * 5.0);
		}
	}
	...
	ord.columns = {oi, ou, ot, oa};
	ord.nrows = no;
	const size_t ni = no * 3;
	Frame it;
	it.name = "items";
	Column ii = MakeCol("id", ColType::INT64, ni), io = MakeCol("order_id", ColType::INT64, ni),
	       iq = MakeCol("qty", ColType::DOUBLE, ni);
	for (size_t i = 0; i < ni; i++) {
		ii.num[i] = double(i);
		io.num[i] = double(i / 3);
		iq.num[i] = 1.0 + U(rng) * 4.0;
	}
	it.columns = {ii, io, iq};
	it.nrows = ni;
	db.tables.push_back(users);
	db.tables.push_back(ord);
	db.tables.push_back(it);
...
static std::vector<unsigned long long> EpochAllocs(const Database &db, const std::string &opts, Model &m) {
	...
	bool first = true;
	auto cb = [&]() {
		const unsigned long long now = g_allocs.load();
		if (!first)
			out.push_back(now - last);
		first = false;
		last = now;
	};
...
int main() {
	...
	}
	{
		...
		for (const char *arch : {"", ", arch = 'sage'"}) {
			Model m;
			TrainModel(db, TrainStmt(std::string("epochs = 2, hidden = 16") + arch), m);
			...
			unsigned long long a0 = g_allocs.load();
			auto p_all = RunPredict(db, m, all);
			unsigned long long a1 = g_allocs.load();
			auto p_few = RunPredict(db, m, few);
			unsigned long long a2 = g_allocs.load();
			printf("   arch '%s': %zu rows -> %llu allocs, %zu rows -> %llu allocs\n", *arch ? "sage" : "mlp",
			       p_all.size(), a1 - a0, p_few.size(), a2 - a1);
			CHECK(p_all.size() > 100 && p_few.size() == 5, "unexpected prediction counts");
			CHECK((a1 - a0) <= (a2 - a1) + 8, "predicting %zu rows allocated %llu more times than predicting 5",
			      p_all.size(), (a1 - a0) - (a2 - a1));
		}
	}
	{
		...
		for (int trial = 0; trial < 2000; trial++) {
			...
			auto carve = [&](Arena &a) {
				...
				for (int i = 0; i < k; i++) {
					switch (i % 3) {
					case 0:
						ptrs.push_back(a.Zeroed<float>(ns[i]));
						break;
					case 1:
						ptrs.push_back(a.Alloc<double>(ns[i]));
						break;
					default:
						ptrs.push_back(a.Alloc<uint32_t>(ns[i]));
						break;
					}
				}
			};
			...
			for (void *p : ptrs)
				CHECK(p == nullptr, "counting mode must hand out no memory");
			Arena a;
			a.Carve(carve);
			CHECK(a.Capacity() == need && a.Used() == need, "Carve: used %zu cap %zu need %zu", a.Used(), a.Capacity(),
			      need);
			...
			for (int i = 0; i < k; i++) {
				...
				for (auto &s : spans)
					CHECK(b + sz <= s.first || s.second <= b, "spans overlap");
		...
		try {
			Arena a;
			a.Reserve(64);
			a.Alloc<double>(std::numeric_limits<size_t>::max() / 4);
		} catch (const std::runtime_error &) {
			threw = true;
		}
		...
		threw = false;
		try {
			Arena::Measure([](Arena &a) { a.Alloc<uint64_t>(std::numeric_limits<size_t>::max() / 2); });
		} catch (const std::runtime_error &) {
			threw = true;
		}
	...
	}
	{
		...
		for (int trial = 0; trial < 300; trial++) {
			...
			for (size_t e = 0; e < E; e++) {
				p.src_dim.push_back(int(rng() % 70));
				p.w_neigh.emplace_back((size_t)p.channels * (size_t)std::max(1, p.src_dim.back()), 0.0f);
			}
			const size_t B = 1 + rng() % 700;
			SageScratch sc;
			sc.Ensure(B, p);
			CHECK(sc.arena.Used() == sc.arena.Capacity(), "SageScratch: used %zu != cap %zu", sc.arena.Used(),
			      sc.arena.Capacity());
			CHECK(sc.xself && sc.z && sc.h && sc.dz && sc.obuf, "SageScratch pointers must be real");
			for (float *mp : sc.means)
				CHECK(mp != nullptr, "means pointer null");
		...
		for (int h : hiddens)
			for (int b : batches)
				for (const char *arch : {"", ", arch = 'sage'", ", arch = 'sage', layers = 2"}) {
					if (h == 130 && *arch)
						continue; // slow, and the scratch path is shape-agnostic
					char opts[160];
					snprintf(opts, sizeof opts, "epochs = 2, hidden = %d, batch = %d%s", h, b, arch);
					Model m;
					try {
						TrainModel(db, TrainStmt(opts), m);
						runs++;
					} catch (const std::exception &ex) {
						CHECK(false, "TrainModel(%s) threw: %s", opts, ex.what());
					}
				}
		...
		size_t data_bytes = 0;
		{
			...
			for (auto &t : db.tables)
				for (auto &c : t.columns)
					data_bytes += c.num.size() * 8 + c.code.size() * 4 + c.valid.size();
