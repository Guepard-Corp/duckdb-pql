...
int main(int argc, char **argv) {
	...
	static const std::string many_nots = [] {
		std::string s;
		for (int i = 0; i < 300; i++)
			s += "NOT ";
		return s;
	...
	for (long iter = 0; iter < iters; iter++) {
		...
		if (iter % 4 == 3) {
			...
			for (int m = 0; m < muts && !tk.empty(); m++) {
				...
				switch (rng() % 4) {
				case 0:
					tk.erase(tk.begin() + (long)at);
					break; // delete
				case 1:
					tk.insert(tk.begin() + (long)at, alphabet[rng() % nalpha]);
					break;
				case 2:
					tk[at] = alphabet[rng() % nalpha];
					break; // replace
				case 3:
					tk.resize(at);
					break; // truncate
