...
struct Rows {
	std::vector<double> uid, uts, ux;
	std::vector<double> eid, euid, ets, eamt;
};
...
static void Try(const char *name, const Database &db, const char *q) {
	try {
		...
		printf("  %-34s trained: %s=%.4f rows=%zu%s\n", name, r.metric_name.c_str(), r.test_metric, r.n_train,
		       bad ? "   <-- INFINITE METRIC" : "");
		if (bad)
			fails++;
		// prediction must survive too
		auto ps =
		    Parse((std::string("PREDICT ") + (st.target.ToString()) + " FOR users USING MODEL " + st.model).c_str());
		...
		if (msg.find("pql:") != 0) {
			printf("      (message does not start with 'pql:')\n");
			fails++;
		}
	}
}
int main() {
	...
	{
		Rows r;
		Try("empty entity table", Make(r), Q);
	}
	{
		Rows r;
		r.uid = {0};
		r.uts = {1000 * DAY};
		r.ux = {1.0};
		Try("one entity row, no events", Make(r), Q);
	}
	{
		Rows r;
		for (int i = 0; i < 50; i++) {
			r.uid.push_back(i);
			r.uts.push_back((1000.0 + i) * DAY);
			r.ux.push_back(1.0);
		}
	...
	}
	{
		...
		for (int i = 0; i < 400; i++) {
			r.uid.push_back(i);
			r.uts.push_back(1000 * DAY);
			r.ux.push_back(7.0);
			r.eid.push_back(i);
			r.euid.push_back(i);
			r.ets.push_back(999 * DAY);
			r.eamt.push_back(3.0);
		}
	...
	}
	{
		...
		for (int i = 0; i < 400; i++) {
			...
			r.eid.push_back(i);
			r.euid.push_back(i);
			r.ets.push_back((999.0 + i) * DAY);
			r.eamt.push_back(1e308);
		}
	...
	}
	{
		Rows r;
		for (int i = 0; i < 400; i++) {
			r.uid.push_back(7); // every entity has the same key
			r.uts.push_back((1000.0 + i) * DAY);
			r.ux.push_back(1.0);
			r.eid.push_back(i);
			r.euid.push_back(7);
			r.ets.push_back((999.0 + i) * DAY);
			r.eamt.push_back(1.0);
		}
	...
	}
	{
		...
		for (int i = 0; i < 2000; i++) {
			r.eid.push_back(i);
			r.euid.push_back(99999); // no parent matches
			r.ets.push_back(999 * DAY);
			r.eamt.push_back(1.0);
		}
	...
	// ---- a classifier with one class must refuse, not train a constant --------
	{
		...
		std::string msg;
		try {
			auto st = Parse("TRAIN MODEL m PREDICT EXISTS(events) FOR users AT ts HORIZON 30 DAYS OPTIONS (epochs = 3, "
			                "hidden = 8)");
			Model m;
			TrainModel(db, st, m);
		} catch (const std::exception &e) {
		...
		printf("  %-34s %s\n", "EXISTS with no positive anywhere",
		       refused ? msg.substr(0, 62).c_str() : "TRAINED (wrong)");
		if (!refused || msg.find("no positives") == std::string::npos) {
			printf("      expected a refusal naming the missing class\n");
			fails++;
		}
	}
	...
	// ---- cancellation fires between minibatches, not only between epochs ------
	{
		...
		bool thrown = false;
		try {
			auto st = Parse("TRAIN MODEL c PREDICT COUNT(events) FOR users AT ts HORIZON 30 DAYS OPTIONS (epochs = 5, "
			                "hidden = 8, batch = 64)");
			Model m;
			TrainModel(db, st, m, [&]() {
				if (++calls == 3)
					throw std::runtime_error("pql: cancelled");
			});
		...
		if (!thrown || calls != 3) {
			printf("      the cancel check must run once per minibatch\n");
			fails++;
		}
		auto st2 = Parse(
		    "TRAIN MODEL c2 PREDICT COUNT(events) FOR users AT ts HORIZON 30 DAYS OPTIONS (epochs = 2, hidden = 8)");
		Model m2;
		TrainModel(db, st2, m2);
		int pcalls = 0;
		bool pthrown = false;
		try {
			RunPredict(db, m2, Parse("PREDICT COUNT(events) FOR users USING MODEL c2"), [&]() {
				if (++pcalls == 1)
					throw std::runtime_error("pql: cancelled");
			});
		} catch (const std::exception &) {
		...
		if (!pthrown) {
			printf("      PREDICT ignored its cancel callback\n");
			fails++;
		}
	}
