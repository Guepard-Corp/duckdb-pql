...
static void Err(const char *sql, const char *want) {
	try {
		pql::Parse(sql);
		std::printf("  FAIL (expected error): %s\n", sql);
		fails++;
	} catch (const std::exception &e) {
		std::string m = e.what();
		if (m.find(want) == std::string::npos) {
			std::printf("  FAIL wrong error: %s\n     got: %s\n     want: %s\n", sql, m.c_str(), want);
			fails++;
		} else
			oks++;
	}
...
int main() {
	...
	Ok("TRAIN MODEL churn PREDICT EXISTS(orders) FOR customers AS c AT c.seen HORIZON 90 DAYS "
	   ...
	   "OPTIONS (epochs = 40, hidden = 128)",
	   "TRAIN MODEL churn");
	Ok("PREDICT COUNT(events WHERE events.status IN ('yes','maybe')) FOR users AT t HORIZON 7 DAYS USING MODEL m",
	   "IN ('yes', 'maybe')");
	...
	Ok("TRAIN MODEL a PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS EXCLUDE (email, o.note)", "EXCLUDE (email, o.note)");
	...
	Err("TRAIN MODEL a PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS EXCLUDE (a) EXCLUDE (b)", "duplicate EXCLUDE");
	...
	Ok("TRAIN MODEL m PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS "
	   "SPLIT TEMPORAL VALIDATE FROM -100 TEST FROM -50",
	   "VALIDATE FROM -100");
	...
	Err("TRAIN MODEL m PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS OPTIONS (l2 = 1000)", "L2 must be between");
	Err("TRAIN MODEL m PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS OPTIONS (l2 = -1)", "L2 must be between");
	...
	Ok("TRAIN MODEL m PREDICT COUNT(o) FOR u AT t HORIZON 5 DAYS "
	   "SPLIT TEMPORAL VALIDATE FROM 100 TEST FROM 200",
	   "VALIDATE FROM 100");
	...
	std::printf("== quotes inside literals survive an echo\n");
	{
		...
		try {
			...
			if (again.ToString() != once) {
				std::printf("  FAIL drift: %s\n", once.c_str());
				fails++;
			} else if (st.filter->children[0]->values[0].text != "it's") {
				std::printf("  FAIL text: %s\n", once.c_str());
				fails++;
			} else
				oks++;
		} catch (const std::exception &e) {
			std::printf("  FAIL echo rejected: %s\n     %s\n", once.c_str(), e.what());
			fails++;
		}
	}
	...
	Ok("PREDICT u.x FOR u WHERE a = .5 AND b = 10.25 USING MODEL m", "10.25");
	{
		...
		std::string deep =
		    "PREDICT u.x FOR u WHERE " + std::string(50000, '(') + "a = 1" + std::string(50000, ')') + " USING MODEL m";
	...
	{
		const char *full = "TRAIN OR REPLACE MODEL m PREDICT COUNT(o WHERE o.kind = 'x') FOR u AS uu "
